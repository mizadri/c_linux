...
int add_element(char* string){

list_item_t *elem = vmalloc(sizeof(list_item_t));
strcpy(elem->data,string);
list_add_tail(&(elem->links),&mylist);

return 0;
}

int remove_element(char *string){
...
static ssize_t modlist_write(struct file *filp, const char __user *buf, size_t len, loff_t *off) {
	...
	*off+=len;           /*Update the file pointer */
	if(sscanf(kbuf,"add %s",n)==1){
		add_element(n);
	}else if(sscanf(kbuf,"remove %s",n)==1){
		remove_element(n);
		...
		printk(KERN_INFO "Argumentos inválidos: add <string> , remove <string>, cleanup, cat");
	}
