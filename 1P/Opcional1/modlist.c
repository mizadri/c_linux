...
int sortList(void){
	struct list_head auxlist;	
	struct list_head *cur_node=NULL;
	struct list_head *next_node=NULL;
	struct list_head *min_node=NULL;
	list_item_t *elem=NULL;
	list_item_t *item_act=NULL;
	list_item_t *item=NULL;
	int valact;
	int min = 100000;
	

	INIT_LIST_HEAD(&auxlist);
	
	while( !list_empty(&mylist)){ 

		list_for_each_safe(cur_node, next_node,&mylist){
			item_act = list_entry(cur_node,list_item_t,links);
			valact = item_act->data;
				if(valact<=min){
					min = valact;
					item = list_entry(cur_node,list_item_t,links);	
					min_node = cur_node;
		...
		list_del_init(min_node);
		vfree(item);
		elem = vmalloc(sizeof(list_item_t));
		elem->data = min;
		list_add_tail(&(elem->links),&auxlist);
		min = 100000;
		
	}
	
	list_splice(&auxlist,&mylist);
	return 0;
}
